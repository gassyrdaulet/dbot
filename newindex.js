  ...
  try {
    ...
    const updatePrices = async ({
        ...
        await Promise.all(
          chunk.map(async (offer) => {
            try {
              ...
              logtext +=
                "SKU=" + offer.id + " scrape failed! Error: " + e + "\n";
              newOffers.push({
                id: offer.id,
      ...
      const updateXML = async () => {
        ...
        await fs.writeFile(
          (production ? XMLFilePathProduction : XMLFilePath) +
            filename +
            ".xml",
          xml
        );
        logtext += "XML updated successfully!";
        await fs.writeFile(LogFilePath + filename + ".txt", logtext);
      };
      updateXML();
    ...
    for (let i = 0; i < Math.ceil(users.length / asyncUserIterations); i++) {
      ...
      await Promise.all(
        ...
        console.log(
          `${finishDate.toLocaleDateString()} ${finishDate.toLocaleTimeString()} CHUNK [${chunk.map(
            (props) => {
              return props.filename;
            }
          )}] SCRAPE END\nTook time: ${(finishDate - startDate) / 1000}s`
        );
        mainlog += `\n${finishDate.toLocaleDateString()} ${finishDate.toLocaleTimeString()} CHUNK [${chunk.map(
          (props) => {
            return props.filename;
